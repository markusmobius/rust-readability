...
pub use html5ever::tokenizer::{states, TokenSink, TokenSinkResult};
pub use html5ever::tokenizer::{CharacterTokens, EOFToken, NullCharacterToken, ParseError};
pub use html5ever::tokenizer::{CommentToken, DoctypeToken, TagToken, Token};
pub use html5ever::tokenizer::{Doctype, EndTag, StartTag, Tag, TagKind};
...
use html5ever::{ns, small_char_set, TokenizerResult};
use log::{debug, trace};
...
use self::macros::{time, unwrap_or_return};
pub use html5ever::buffer_queue::{BufferQueue, FromSet, NotFromSet, SetResult};
use html5ever::tendril::StrTendril;
use html5ever::{Attribute, LocalName, QualName, SmallCharSet};

mod char_ref;
mod macros;
mod scanner;

fn lower_ascii_letter(character: char) -> Option<char> {
    character
        .is_ascii_alphabetic()
...
impl<Sink: TokenSink> Tokenizer<Sink> {
    ...
    fn append_name_prefix(&self, input: &BufferQueue, output: &RefCell<StrTendril>) {
        if self.opts.exact_errors || self.reconsume.get() || self.ignore_lf.get() {
            return;
        }
        let Some(mut chunk) = input.peek_front_chunk_mut() else {
            return;
        };
        let length = chunk
            .bytes()
            .take_while(|byte| matches!(byte, b'a'..=b'z' | b'0'..=b'9' | b'-' | b':' | b'_'))
            .count();
        if length == 0 {
            return;
        }
        self.current_char
            .set(char::from(chunk.as_bytes()[length - 1]));
        output.borrow_mut().push_slice(&chunk[..length]);
        chunk.pop_front(length as u32);
        if chunk.is_empty() {
            drop(chunk);
            input.pop_front();
        }
    ...
    fn pop_except_from(&self, input: &BufferQueue, set: SmallCharSet) -> Option<SetResult> {
        ...
        let d = scanner::pop_except_from(input, set);
...
impl<Sink: TokenSink> Tokenizer<Sink> {
    ...
    fn step(&self, input: &BufferQueue) -> ProcessResult<Sink::Handle> {
        ...
        match self.state.get() {
            ...
            //§ tag-name-state
            states::TagName => loop {
                self.append_name_prefix(input, &self.current_tag_name);
                match get_char!(self, input) {
            ...
            states::AttributeName => loop {
                self.append_name_prefix(input, &self.current_attr_name);
                match get_char!(self, input) {
    ...
    fn dump_profile(&self) {
        let mut results: Vec<(states::State, u64)> = self
            ...
            .map(|(s, t)| (*s, *t))
            .collect();
        results.sort_by_key(|&(_, duration)| std::cmp::Reverse(duration));
...
mod test {
    ...
    use html5ever::tokenizer::{EndTag, StartTag, Tag, TagKind};
    use html5ever::tokenizer::{TagToken, Token};
    ...
    #[test]
    fn bulk_names_match_released_tokenizer() {
        fn record(chunks: &[StrTendril], exact_errors: bool, released: bool) -> Vec<(Token, u64)> {
            struct Recorder(RefCell<Vec<(Token, u64)>>);
            impl TokenSink for Recorder {
                type Handle = ();

                fn process_token(&self, token: Token, line: u64) -> TokenSinkResult<()> {
                    self.0.borrow_mut().push((token, line));
                    TokenSinkResult::Continue
                }
            }
            let input = BufferQueue::default();
            ...
            if released {
                let tokenizer = html5ever::tokenizer::Tokenizer::new(
                    sink,
        ...
        for source in [
            "<custom-element data-value='123' xml:lang=\"en\" _private=x></custom-element>",
            "<DIV CLASS=wide data-MixedCase=x></DIV>",
            "<a \r\nlongattribute=value\r\nhref='/test'>text</a>",
            "<x\u{e9} data-\u{20ac}=yes>text</x\u{e9}>",
            "<input checked disabled\tvalue=abc/>",
            "<x\0y a\0b=value><x a\"b=1 a'b=2 a<b=3>",
            "<x a=1 a=2></x illegal=value>",
            "<x\r\nlong-attribute=value / >",
            "<custom-element long-attribute",
            "<x longattribute='unfinished",
            "\u{feff}<x\0 a=\0>",
            "<x\x01y data-\u{85}=yes>\u{fdd0}</x\x01y>",
        ] {
            for split in (0..=source.len()).filter(|&position| source.is_char_boundary(position)) {
                let chunks = vec![source[..split].into(), source[split..].into()];
                for exact_errors in [false, true] {
                    assert_eq!(
                        record(&chunks, exact_errors, false),
                        record(&chunks, exact_errors, true),
                        "{source:?} at {split}, exact_errors={exact_errors}"
                    );
                }
            }
        }
