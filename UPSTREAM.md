...
The complete 0.6.0 numeric record, including all samples, source/binary hashes and the seven identical empty-extraction cases, is [testdata/benchmark-results-0.6.0.json](testdata/benchmark-results-0.6.0.json), SHA-256 `7204a53f119d2d94c523d5706236304a2a5a6c2326687d0c7df018ab4b51cd04`. Its measured source bytes match the repository's LF checkout. The Go repository retains a byte-identical copy. Full article outputs and build copies stay under ignored `target/benchmark/`; source pages are not repackaged or relicensed. Scorer/tooling attribution is retained in [licenses/LICENSE-benchmark.txt](licenses/LICENSE-benchmark.txt).
...
An earlier 102-pass run is retained in [testdata/benchmark-results-0.6.0-crlf.json](testdata/benchmark-results-0.6.0-crlf.json), SHA-256 `b245fc3c91ace7d39f91e883054bf468750e8693c8800985b9dca7490cc3c312`, with a byte-identical Go copy. Four build inputs had local CRLF bytes that Git normalized to LF. The final run above repeated the same protocol after line-ending normalization so its fingerprints reproduce from a checkout. No extraction logic changed between these runs; their samples are not combined.
